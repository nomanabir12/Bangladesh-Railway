...
public class BR_Forget_Password extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					BR_Forget_Password frame = new BR_Forget_Password();
	...
	private JTextField reset_email;
	private JTextField reset_phone;
	
	public BR_Forget_Password() {
		...
		JLabel lblStationFrom = new JLabel("Phone Number");
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try
				{
					String query = "select * from login where Email=? and Phone=?";
					PreparedStatement pst = connection.prepareStatement(query);
					pst.setString(1, reset_email.getText());
					pst.setString(2, reset_phone.getText());
					...
					if(count == 1)
					{
						...
						dispose();
						BR_Reset_Password new_pass = new BR_Reset_Password();
		...
		reset_email = new JTextField();
		reset_email.setBounds(67, 59, 306, 36);
		panel_1.add(reset_email);
		reset_email.setColumns(10);
		
		reset_phone = new JTextField();
		reset_phone.setColumns(10);
		reset_phone.setBounds(67, 138, 306, 36);
		panel_1.add(reset_phone);
		...
		JLabel lblNewLabel_4 = new JLabel("If You have no Account With us, Please");
